...
const saveCar = async (req: NextApiRequest, res: NextApiResponse) => {
    const { brand, model, year, color, mileage, category, price, imageFormat, imageData } = req.body;
	...
	const newCar: carType = { brand, model, year, img, color, mileage, category, price: parseFloat(price)};

    prisma.cars_for_sale.create({ data: newCar })
    .then((result) => res.send(result))
	.catch((error) => res.status(500).send("Error: " + error.message))
}

const getCars = async (req: NextApiRequest, res: NextApiResponse) => {
    prisma.cars_for_sale.findMany()
